...
static void CopyPlane(const uint8_t* srcY, int srcStrideY, uint8_t* dstY,
                    int dstStrideY, int width, int height) {
...
static void MergeUVRow(const uint8_t* srcU, const uint8_t* srcV, uint8_t* dstUv, int width) {
...
void convertYUV420Planar8ToNV12(const uint8_t* srcY,int srcStrideY, const uint8_t* srcU,
                                int srcStrideU, const uint8_t* srcV, int srcStrideV, uint8_t* dstY,
                                int dstStrideY, uint8_t* dstUv, int dstStrideUv, int width, int height) {
    if (!srcY || !srcU || !srcV || !dstY || !dstUv || width <= 0 || height == 0) {
        return;
    }
    ...
    for (int y = 0; y < halfHeight; ++y) {
        ...
        MergeUVRow(srcU, srcV, dstUv, halfWidth);
        srcU += srcStrideU;
        srcV += srcStrideV;
        dstUv += dstStrideUv;
    }
...
void convertYUV420Planar8ToNV21(const uint8_t* srcY, int srcStrideY, const uint8_t* srcU,
                                int srcStrideU, const uint8_t* srcV, int srcStrideV, uint8_t* dstY,
                                int dstStrideY, uint8_t* dstVu, int dstStrideVu, int width, int height) {
  return convertYUV420Planar8ToNV12(srcY, srcStrideY, srcV, srcStrideV, srcU,
                    srcStrideU, dstY, dstStrideY, dstVu, dstStrideVu,
                    width, height);
}

std::unique_ptr<C2Work> C2SoftImageComponent::WorkQueue::pop_front() {
    std::unique_ptr<C2Work> work = std::move(mQueue.front().work);
    mQueue.pop_front();
    return work;
}

void C2SoftImageComponent::WorkQueue::push_back(std::unique_ptr<C2Work> work) {
    mQueue.push_back({ std::move(work), NO_DRAIN });
}

bool C2SoftImageComponent::WorkQueue::empty() const {
    return mQueue.empty();
}

void C2SoftImageComponent::WorkQueue::clear() {
    mQueue.clear();
}

uint32_t C2SoftImageComponent::WorkQueue::drainMode() const {
    return mQueue.front().drainMode;
}

void C2SoftImageComponent::WorkQueue::markDrain(uint32_t drainMode) {
    mQueue.push_back({ nullptr, drainMode });
}

////////////////////////////////////////////////////////////////////////////////

C2SoftImageComponent::WorkHandler::WorkHandler() : mRunning(false) {}

void C2SoftImageComponent::WorkHandler::setComponent(
        const std::shared_ptr<C2SoftImageComponent> &thiz) {
    mThiz = thiz;
}
...
void C2SoftImageComponent::WorkHandler::onMessageReceived(const sp<AMessage> &msg) {
    std::shared_ptr<C2SoftImageComponent> thiz = mThiz.lock();
    if (!thiz) {
        CODEC2_LOG(CODEC2_LOG_INFO, "Component not yet set; msg = %s", msg->debugString().c_str());
        sp<AReplyToken> replyId;
    ...
    CODEC2_LOG(CODEC2_LOG_INFO, "Component recv; msg = %s", msg->debugString().c_str());
    switch (msg->what()) {
        case kWhatProcess: {
            if (mRunning) {
                ...
                CODEC2_LOG(CODEC2_LOG_INFO, "Ignore process message as we're not running");
            }
            CODEC2_LOG(CODEC2_LOG_INFO,"end process");
            break;
        }
        case kWhatInit: {
        ...
        case kWhatStop: {CODEC2_LOG(CODEC2_LOG_INFO,"stop process");
            int32_t err = thiz->onStop();
            thiz->mOutputBlockPool.reset();
            Reply(msg, &err);
            break;
        }
        case kWhatReset: {CODEC2_LOG(CODEC2_LOG_INFO,"kWhatReset process");
            thiz->onReset();
            thiz->mOutputBlockPool.reset();
            mRunning = false;
            Reply(msg);
        ...
        case kWhatRelease: {CODEC2_LOG(CODEC2_LOG_INFO,"kWhatRelease process");
            thiz->onRelease();
            thiz->mOutputBlockPool.reset();
            mRunning = false;
            Reply(msg);
        ...
        default: {
            CODEC2_LOG(CODEC2_LOG_INFO, "Unrecognized msg: %d", msg->what());
            break;
        }
    }
}

class C2SoftImageComponent::BlockingBlockPool : public C2BlockPool {
public:
    ...
    virtual uint64_t getConsumerUsage() {
        uint64_t usage = 0;
...
C2SoftImageComponent::C2SoftImageComponent(
...
c2_status_t C2SoftImageComponent::setListener_vb(
...
c2_status_t C2SoftImageComponent::queue_nb(std::list<std::unique_ptr<C2Work>> * const items) {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
    {
        Mutexed<ExecState>::Locked state(mExecState);
        if (state->mState != RUNNING) {
            CODEC2_LOG(CODEC2_LOG_ERR, "%s unexpected state: %d", __func__, state->mState);
            return C2_BAD_STATE;
        }
    }
...
c2_status_t C2SoftImageComponent::announce_nb(const std::vector<C2WorkOutline> &items) {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
    (void)items;
    return C2_OMITTED;
}

c2_status_t C2SoftImageComponent::flush_sm(
    ...
    // Wait until current processQueue finished.
    Mutex::Autolock _l(mProcessQueueLock);
    return C2_OK;
}

c2_status_t C2SoftImageComponent::drain_nb(drain_mode_t drainMode) {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
...
c2_status_t C2SoftImageComponent::start() {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
    Mutexed<ExecState>::Locked state(mExecState);
    if (state->mState == RUNNING) {
        return C2_BAD_STATE;
...
c2_status_t C2SoftImageComponent::stop() {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
    {
        Mutexed<ExecState>::Locked state(mExecState);
        if (state->mState != RUNNING) {
...
c2_status_t C2SoftImageComponent::reset() {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
    {
        Mutexed<ExecState>::Locked state(mExecState);
...
c2_status_t C2SoftImageComponent::release() {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
    sp<AMessage> reply;
...
std::shared_ptr<C2ComponentInterface> C2SoftImageComponent::intf() {
    return mIntf;
}
...
void C2SoftImageComponent::finish(
        uint64_t frameIndex, uint64_t customOrdinal, std::function<void(const std::unique_ptr<C2Work> &)> fillWork) {
    std::unique_ptr<C2Work> work;
    {
        Mutexed<WorkQueue>::Locked queue(mWorkQueue);
        ALOGE("queue->pending().count(frameIndex) %d");
        if (queue->pending().count(frameIndex) == 0) {
            CODEC2_LOG(CODEC2_LOG_ERR, "Unknown frame index: %" PRIu64, frameIndex);
            return;
        }
        work = std::move(queue->pending().at(frameIndex));
        queue->pending().erase(frameIndex);
    }
    CODEC2_LOG(CODEC2_LOG_INFO, "finish pending work");
    if (work) {
        CODEC2_LOG(CODEC2_LOG_INFO, "enter pending work");
        // Set out pts
        ...
        CODEC2_LOG(CODEC2_LOG_INFO, "Returning pending work");
    }
}

void C2SoftImageComponent::cloneAndSend(
        uint64_t frameIndex,
    ...
    listener->onWorkDone_nb(shared_from_this(), vec(work));
    CODEC2_LOG(CODEC2_LOG_INFO, "Cloned and sending work");
}

bool C2SoftImageComponent::processQueue() {
    std::unique_ptr<C2Work> work;
    uint64_t generation;
    int32_t drainMode;
    bool isFlushPending = false;
    bool hasQueuedWork = false;
    CODEC2_LOG(CODEC2_LOG_INFO, "processQueue");
    Mutex::Autolock _l(mProcessQueueLock);
    {
        Mutexed<WorkQueue>::Locked queue(mWorkQueue);
        if (queue->empty()) {
    ...
    if (isFlushPending) {
        ...
        if (err != C2_OK) {
            CODEC2_LOG(CODEC2_LOG_ERR, "Flush err: %d", err);
            // TODO: error
        }
    }
    CODEC2_LOG(CODEC2_LOG_INFO, "mOutputBlockPool");
    if (!mOutputBlockPool) {
        c2_status_t err = [this] {
            ...
            if (err != C2_OK && err != C2_BAD_INDEX) {
                CODEC2_LOG(CODEC2_LOG_ERR, "Query err = %d", err);
                return err;
            }
            ...
            CODEC2_LOG(CODEC2_LOG_INFO, "Using output block pool with poolID %llu => got %llu - %d",
            ...
            if (err == C2_OK) {
                ...
                if (mOutputBlockPool->getAllocatorId() == C2PlatformAllocatorStore::BUFFERQUEUE) {
                    surfaceUsage = mOutputBlockPool->getConsumerUsage();
                ...
                if (!usersurfacetexture && !noSurface) {
                    ALOGE("mPlatformUsage settings!!!!!!!!!!");
                    mPlatformUsage = am_gralloc_get_video_decoder_full_buffer_usage();
                    mPlatformUsage = mPlatformUsage & C2MemoryUsage::PLATFORM_MASK;
                }
            }
            return err;
        }();
        CODEC2_LOG(CODEC2_LOG_INFO, "err != C2_OK %d",(err != C2_OK));
        if (err != C2_OK) {
            Mutexed<ExecState>::Locked state(mExecState);
...

    {
        ...
        if (!updates.empty()) {
            ...
            CODEC2_LOG(CODEC2_LOG_INFO, "Applied %zu configUpdates => %s (%d)", updates.size(), asString(err), err);
        }
    }

    CODEC2_LOG(CODEC2_LOG_INFO, "Start processing frame #%" PRIu64, work->input.ordinal.frameIndex.peeku());
    ...
    if (!work->input.buffers.empty() && !work->input.buffers[0]) {
        CODEC2_LOG(CODEC2_LOG_INFO, "Encountered null input buffer. Clearing the input buffer");
    ...
    if (queue->generation() != generation) {
        CODEC2_LOG(CODEC2_LOG_INFO, "Work form old generation: was %" PRIu64 " now %" PRIu64,
...
int C2SoftImageComponent::getHalPixelFormatForBitDepth10(bool allowRGBA1010102) {
...
std::shared_ptr<C2Buffer> C2SoftImageComponent::createLinearBuffer(
        const std::shared_ptr<C2LinearBlock> &block, size_t offset, size_t size) {
    return C2Buffer::CreateLinearBuffer(block->share(offset, size, ::C2Fence()));
}

std::shared_ptr<C2Buffer> C2SoftImageComponent::createGraphicBuffer(
