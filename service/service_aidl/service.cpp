...
static constexpr char kBaseSeccompPolicyPath[] =
        "/vendor/etc/seccomp_policy/android.hardware.amlogic.media.c2-seccomp-policy";


// Additional seccomp permissions can be added in this file.
// This file does not exist by default.
static constexpr char kExtSeccompPolicyPath[] =
        "/vendor/etc/seccomp_policy/android.hardware.amlogic.media.c2-extended-seccomp-policy";
...
void runAidlService() {
    ...
    ALOGI("Instantiating Codec2's Vendor IComponentStore service...");
    store = ::ndk::SharedRefBase::make<utils::ComponentStore>(android::GetCodec2VendorComponentStore());
    ...
    if (store == nullptr) {
        ...
        if (ex != EX_NONE) {
            ...
            ALOGI("Codec2's IComponentStore service registered. "
...
void runHidlService() {
    ...
    // Create IComponentStore service.
    {
        ...
        if (store == nullptr) {
           ALOGI("Cannot create Codec2's Vendor IComponentStore service.");
        } else {
            constexpr char const* serviceName = "default";
            if (store->registerAsService(serviceName) != android::OK) {
                ALOGE("Cannot register Codec2's IComponentStore service."
                            " with instance name %s", serviceName);
            } else {
                ALOGI("Codec2's IComponentStore default service created.");
            }
        }
    }
...
int main(int /* argc */, char** /* argv */) {
    ...
    ALOGI("android.hardware.media.c2%s -service starting...",
           (aidlEnabled ? "-V1" : "@1.2"));
